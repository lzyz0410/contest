...
def energy_based_correction(transformed_points_second, second_transition_filtered_points, collision_set, alpha=0.5, beta=1.0, max_step=0.1):
    """
    对第二次 RBF 变换后的点进行碰撞检测，仅调整穿透的点，其他点保持不变。
    ...
    - alpha: 平滑因子，决定势能优化与原始变换趋势的融合程度
    - beta: 势能函数控制参数，影响斥力作用范围
    - max_step: 限制斥力步长，防止点跳跃过远

    返回:
    - corrected_points: np.array (n, 4), 经过碰撞检测调整的点
    """
    ...
    # 仅调整碰撞点，未碰撞的点保持原样
    collision_mask = np.zeros(len(corrected_points), dtype=bool)  # 用于标记哪些点需要调整
    ...
    for i in range(len(corrected_points)):
        ...
        if distance < 1e-6:
            continue  # 避免除零错误
        if distance < max_step:  # **距离太近，认为发生穿透**
            collision_mask[i] = True  # 记录这个点发生碰撞

            # 计算斥力方向
            direction = (p_prime - closest_point) / distance  # 归一化方向
            phi = alpha * np.exp(-beta * distance**2)  # 指数势能计算

            # 计算势能带来的偏移
            energy_correction = min(phi, max_step) * direction

            # 结合原始形变趋势 (original_movement) 和势能优化 (energy_correction)
            final_movement = (1 - alpha) * original_movement[i] + alpha * energy_correction
